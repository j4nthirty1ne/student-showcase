            ...
            <form action="{{ route('student.projects.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="mb-6">
                    ...
                    <input type="url" id="github_link" name="github_link"
                        ...
                        value="{{ old('github_link') }}" placeholder="https://github.com/your-username/repo">
                    @error('github_link')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    @error('cover_image')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-gray-500 text-sm mt-1">Recommended: Upload a clear image to represent your project.</p>
                </div>

                <!-- Gallery Images -->
                <div class="mb-6">
                    <label for="gallery_images" class="block text-gray-700 font-semibold mb-2">Project Gallery (Multiple
                        Images)</label>
                    ...
                    @error('gallery_images.*')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-gray-500 text-sm mt-1">Select multiple images to show off your project.</p>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="technologies" name="technologies"
                        ...
                        placeholder="e.g., Laravel, Vue.js, MySQL, Tailwind CSS (comma-separated)" required>
                    @error('technologies')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
